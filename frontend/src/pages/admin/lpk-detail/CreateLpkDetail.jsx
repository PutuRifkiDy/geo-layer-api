...
import { Button, Input, Switch, message as messageAntd } from "antd"; // Menggunakan alias agar tidak bentrok dengan nama variabel
import { Link, useNavigate, useParams } from "react-router-dom";
import { createLpkDetail } from "../../../api/lpk-detail";

function CreateLpkDetail() {
  ...
  const onHandleSubmit = async (event) => {
    ...
    if (!pointObjectId) {
      error("ID Point Object tidak ditemukan di URL.");
      return;
    }
    ...
    try {
      ...
      const response = await createLpkDetail(data, pointObjectId);

      if (response?.status === "berhasil") {
        success(response.message || "LPK Detail berhasil dibuat");
        setTimeout(() => {
          navigate(`/dashboard/lpk-details/${pointObjectId}`);
        }, 1000);
      } else {
        error(response?.message || "Gagal membuat LPK detail baru");
      }
    } catch (err) {
      console.log(err);
      ...
      <form className="mt-4 space-y-6" onSubmit={onHandleSubmit}>
        <div className="flex justify-between items-center">
          <div className="space-y-1">
            <h2 className="text-2xl font-bold text-gray-800">Tambah LPK Detail</h2>
            <p className="text-sm text-gray-500">Lengkapi informasi detail LPK di bawah ini.</p>
          </div>
          <Button type="default" className="bg-gray-200 text-gray-700 hover:bg-gray-300" onClick={() => navigate(`/dashboard/lpk-details/${pointObjectId}`)}>
            Kembali
          </Button>
        </div>
...
export default CreateLpkDetail;